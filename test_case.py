...
class pyBoard:
    ...
    def delete_article(self, delete_target):
        for article in self.article_list :
            if article == delete_target :
                self.article_list.remove(delete_target)
            ...
            if cmd == '1':
                ...
                if exist_rs :
                    if self.is_my_article(exist_rs, user) :
                        ...
                        else : print("*** '게시물번호', '작성자'는 수정 불가능합니다.")
                    else : print('*** 게시물을 수정할 권한이 없습니다.')
                else : print('*** 존재하지 않는 게시물 입니다.')

            elif cmd == '4':
                print('\n▷ 게시물삭제 모드')
                artc_no = int(input('--- 삭제할 게시물 번호를 입력해주세요 : '))
                exist_rs = self.is_exist_article(artc_no)
                if exist_rs :
                    if self.is_my_article(exist_rs, user) :
                        self.delete_article(exist_rs)
                        print('>>> 게시물이 삭제 되었습니다.')
                    else : print('*** 게시물을 삭제할 권한이 없습니다.')
                else : print('*** 존재하지 안흔ㄴ 게시물 입니다.')
